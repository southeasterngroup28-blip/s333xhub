...
export type Comment = {
  ...
  author: { display_name: string; role: string; status: string | null } | null;
};

const COMMENT_SELECT =
  'id, post_id, user_id, body, pinned, created_at, deleted_at, author:profiles!post_comments_user_id_fkey(display_name, role, status)';
...
export async function fetchTopFans(): Promise<TopFan[]> {
  const { data, error } = await supabase
    .from('top_fans')
    .select('position, user_id, profile:profiles!top_fans_user_id_fkey(display_name)')
    .order('position');
  if (error) throw error;
